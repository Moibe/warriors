<script lang="ts">
  ...
  let {
    ...
    /**
     * Other units standing inside the burst. The numbers above describe the
     * unit under the cursor only, so an area ability has to say out loud that
     * it is about to touch more than that.
     */
    alsoCaught?: number;
  } = $props();
...
<Window title={data.healing ? 'Curación' : 'Previsión'}>
  ...
  {#if alsoCaught > 0}
    <p class="area">
      El área alcanza a {alsoCaught}
      {alsoCaught === 1 ? 'unidad más' : 'unidades más'}.
    </p>
  {/if}
